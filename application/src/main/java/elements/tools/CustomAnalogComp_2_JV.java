...
public class CustomAnalogComp_2_JV extends AbstractCustomAnalogComp {
    ...
    public void initSubElements() {
        ...
        // double vectLen=(double) (vectorLaenge+abstand.getValue()+ w*5/100 +((w-(w*5/100)*2)*0.3));
        // //(Distancia circulo + Distancia texto)

        // double vectLen1=vectorLaenge+7.0;
        double vectLen1 = vectorLaenge;

        if (textInside.getValue()) { // vectorLaenge=(Math.sqrt((x1*x1)+(y1*y1))-3*TextGap+5);
            TextGap = (int) (w * 0.28);
    ...
    public void drawAnzeige(java.awt.Graphics gx, int x, int y, int w, int h) {
        ...
        for (int i = 0; i < listeBeschriftungen.size(); i++) {
            Beschriftung<CustomAnalogComp_2_JV> bes = (Beschriftung) listeBeschriftungen.get(i);
        ...
        if (showText.getValue()) // Dibujar Arco sobre el que van las líneas de división
        {
            ...
            // g.drawArc(distance,distance,w-distance*2+2,h-distance*2+2,
            // 180-(int)minGrad.getValue(),-(int)maxGrad.getValue());
            if (showText.getValue() && textInside.getValue()) {
                g.drawArc(w / 2 - (TempW / 2), h / 2 - (TempH / 2), TempW, TempH, 180 - (int) minGrad.getValue(),
                        -(int) maxGrad.getValue());
            } else {

                g.drawArc(w / 2 - (Tam / 2), h / 2 - (Tam / 2), Tam, Tam, 180 - (int) minGrad.getValue(),
                        -(int) maxGrad.getValue());
        ...
        if (showText.getValue()) {
            ...
            if (textInside.getValue()) {
                ...
                vectorLaenge =
                        ((Math.sqrt((x1 * x1) + (y1 * y1)) - (double) d2) - (InsideGapW / 2) - 12) + abstand.getValue();
                vectLen = vectorLaenge - abstand.getValue();
                vectLen1 = vectorLaenge - (double) d2;
            }
    ...
    public void generateFromValuesSubElements() {
        ...
        for (int i = 0; i < values.getLength(); i++) {
            ...
            Beschriftung<CustomAnalogComp_2_JV> bes = new Beschriftung(this, strValue);
