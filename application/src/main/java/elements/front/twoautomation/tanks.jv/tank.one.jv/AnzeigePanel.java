...
package elements.front.twoautomation.tanks.jv.tank.one.jv;/*
                                                                                 * Copyright (C) 2020 MyLibreLab
        ...
        try {
            if (element != null) {
                ...
                if (element.jGetWidth() <= 250.0) {
                    ...
                    slider.setFont(new Font(fnt.getValue().getFontName(), fnt.getValue().getStyle(),
                            (fnt.getValue().getSize())));
                }
                if (element.jGetWidth() > 250.0) {
                    slider.setMinimumSize(new Dimension(50, 110));
                ...
                if (value >= 0) {
                    GradientPaint gp = new GradientPaint(0.0f, 50.0f, fillColor1.getValue(), (float) bounds.width,
                            50.0f, fillColor2.getValue());
                    g2.setPaint(gp);
                    g2.fillArc((X0 + strokeValue.getValue() / 2), YMin, tWidth, tHeight, 180, 180);
                    g2.fillRoundRect((X0 + strokeValue.getValue() / 2), Y0 + (tHeight / 2), tWidth,
                            (YMin - Y0) + strokeValue.getValue(), arcValue.getValue(), arcValue.getValue());
                }
                if (value == 0)
                    overlayColor.setFillColor(g2);
                else
                    UpperFillColor.setFillColor(g2);
