...
package elements.front.twoautomation.valves.jv.down.valve.jv;/*
                                                                                    * Copyright (C) 2020 MyLibreLab
...
public class LEDPanel extends JVSMain implements PanelIF {
    ...
    public void paint(java.awt.Graphics g) {
        if (element != null) {
            ...
            if (manualValve.getValue() == false) {
                ...
                tHeight = ((element.jGetHeight() - ((ValveSize + stroke.getValue()))) + stroke.getValue()) / 2;
                Y0 = (element.jGetHeight() / 2);
                // Y0-=tHeight;
                ...
                g2.setStroke(new BasicStroke(stroke.getValue()));
                g2.fillRect(X0 + (3 * tWidth / 8), Y0, tWidth / 4, tHeight);

                if (overlay)
                    overlayColor.setFillColor(g2);
                else
                    StrokeColor.setFillColor(g2);
                g2.drawRect(X0 + (3 * tWidth / 8), Y0, tWidth / 4, tHeight);
            ...
            tWidth = ((element.jGetWidth() - ((BorderSize + stroke.getValue())))) - (X0 + (stroke.getValue() / 2));
            ...
            p.addPoint(X0 + (tWidth / 2), Y0 + (tHeight / 2) - tHeight1);
            p.addPoint(X0 + (tWidth / 2), Y0 + (tHeight / 2) + tHeight1);
            p.addPoint(X0, Y0 + (tHeight));
            p.addPoint(X0, Y0);
            ...
            p1.addPoint(X0 + (tWidth / 2), Y0 + (tHeight / 2) - tHeight1);
            p1.addPoint(X0 + (tWidth / 2), Y0 + (tHeight / 2) + tHeight1);
            p1.addPoint(X0 + (tWidth), Y0 + (tHeight));
            p1.addPoint(X0 + (tWidth), Y0);
            ...
            // --------------------------------------------------------------------FINAL SECCIÓN DE TUBO IZQ -
            // DERECHA

            // --------------------------------------------------------------------------INICIO CONECTOR DERECHA
            BorderSize = cnxSize.getValue();
            X0 = stroke.getValue() / 2;
            ...
            if (manualValve.getValue() == false) {
                tHeight = ValveSize + stroke.getValue();
                tWidth = (element.jGetWidth() - stroke.getValue()) - (element.jGetWidth() / 2);
                Y0 = (element.jGetHeight()) - (stroke.getValue() / 2) - tHeight;
                X0 = stroke.getValue() / 2 + (element.jGetWidth() / 4);

                if (on)
                    ValveOnColor.setFillColor(g2);
                else
                    ValveOffColor.setFillColor(g2);

                g2.setStroke(new BasicStroke(stroke.getValue()));

                if (Diaphragm.getValue()) {
                    g2.fillArc(X0, Y0 - tHeight, tWidth, tHeight * 2, 0, -180);
                } else {
                ...
                if (Diaphragm.getValue()) {
                    g2.drawArc(X0, Y0 - tHeight, tWidth, tHeight * 2, 0, -180);
                } else {
                    if (roundValve.getValue()) {
    ...
    public void init() {
        ...
        offColor.modus = 1;
        offColor.p1 = new Point(25, 0);
        offColor.p2 = new Point(25, 35);
