...
package elements.front.tworobotics.simplerobotthreed;/*
                                                                            * Copyright (C) 2020 MyLibreLab
...
public class Panel extends JVSMain implements PanelIF {
    ...
    public void processPanel(int pinIndex, double value, Object obj) {
        ...
        if (panelX != null) {
            ...
            } else if (pinIndex == 0) // Reinit
            {
                panelX.reinit();
            } else if (pinIndex == 1 && obj instanceof Double) // Go Forward /Back
            {
                Double tmp = (Double) obj;
                int robotNr = (int) value;
                panelX.setBasisPos(robotNr, tmp.doubleValue(), 0, 0, 0);
            } else if (pinIndex == 2 && obj instanceof Double) // Turn Left / Right
            {
                Double tmp = (Double) obj;
                int robotNr = (int) value;
            ...
            } else if (pinIndex == 3 && obj instanceof ArrayList) // Get Robot Info
            {
                ...
                if (liste.size() == 2) {
                    ...
                    switch (getNr) {
                        case 0:
                            res = panelX.getPositionX(robotNr);
                            break;
                        case 1:
                            res = panelX.getPositionY(robotNr);
                            break;
                        case 2:
                            res = panelX.getNearstPickableObjectDistance(robotNr);
                            break;
                        case 3:
                            res = panelX.getSensorValue(robotNr, 1);
                            break;
                        case 4:
                            res = panelX.getSensorValue(robotNr, 2);
                            break;
                        case 5:
                            res = panelX.getSensorValue(robotNr, 3);
                            break;
                    }
