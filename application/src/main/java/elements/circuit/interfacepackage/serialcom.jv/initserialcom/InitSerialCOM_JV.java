...
public class InitSerialCOM_JV extends JVSMain {
    ...
    public void paint(java.awt.Graphics g) { // element.jConsolePrintln("Paint");
        if (image != null) {
    ...
    public void init() { // element.jConsolePrintln("Init!");
    ...
    public void initInputPins() { // element.jConsolePrintln("Init Input Pins!");
    ...
    public void initOutputPins() { // element.jConsolePrintln("Init Output Pins!");
    ...
    public void start() { // element.jConsolePrintln("start!");
    ...
    public void stop() { // element.jConsolePrintln("Stop");
        String ErrorData = "";
        Error_out.setValue(false);
    ...
    public void process() { // element.jConsolePrintln("Proccess");
        ...
        if (EnableProccess == true && Error_in.getValue() == false && PortOpened == false) // Only executes one time if
                                                                                           // no error.
        {
                ...
                try {
                    ...
                    if (PortOpened == false) {
                        ...
                        if (Debug_Window_En_in.getValue()) {
                            element.jConsolePrintln(Element_Tag + "|Port:_" + vsSerial.getSerialPort().getPortName()
                                    + "_Opened OK_Total Ports:_" + NewSerialDriverManager.getPortsOpened() + "|");
                        }
                        vsSerial.setParameters(ArduinoAutoRESET.getValue(), Thread.currentThread(),
                                AutoResetTime.getValue(), BaudRateTemp, dataBitsTemp, stopBitsInt, ParityTemp);
                        NewSerialDriverManager.UpdateSerialPortByPortName(vsSerial, SerialPortNameIn.getValue());
                    }
                    Error_out.setValue(false);

                    if (Debug_Window_En_in.getValue()) {
                        element.jConsolePrintln(Element_Tag + "|Port_Configured_OK_" + "Baud:" + BaudRateTemp
                                + "_dataBits:" + dataBitsTemp + "_StopBits:" + stopBitsInt + "_Paridad:"
                    ...
                    if (Debug_Window_En_in.getValue() && Error_out.getValue()) {
                        element.jConsolePrintln(Element_Tag + Error_Tag + "|ErrorOpeningPort:"
                                + SerialPortNameIn.getValue() + BufferError);
        ...
        if (Error_out.getValue() || Error_in.getValue() || PortOpened == false) { // If there is error do not enable
                                                                                  // other VMs
            Enable_VM_out.setValue(false);
            element.notifyPin(1); // Enable VM Out
        } else {
    ...
    public void setPropertyEditor() { // element.jConsolePrintln("Set Property Editor!");
    ...
    private void localize() { // element.jConsolePrintln("Localize");
        String language;
        int d = 6;
        language = "en_US";
