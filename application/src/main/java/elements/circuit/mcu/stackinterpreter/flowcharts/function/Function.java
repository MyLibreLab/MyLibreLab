...
package elements.circuit.mcu.stackinterpreter.flowcharts.function;// *****************************************************************************
...
public class Function extends MCUMainFlow {
    ...
    private final ArrayList<Function_Item> liste = new ArrayList<Function_Item>();
    private final VSComboBox function = new VSComboBox();
    private final VSString paramA = new VSString("");
    private final VSString paramB = new VSString("");
    private final VSString result = new VSString("");
    private Image image;
    ...
    public void paint(java.awt.Graphics g) {
        if (element != null) {
            ...
            if (item.paramCount == 0) {
                caption += funcName + "()";
            } else if (item.paramCount == 1) {
                caption += funcName + "(" + paramA.getValue() + ")";
            } else if (item.paramCount == 2) {
    ...
    private String parseParamA(Function_Item item) {
        ...
        } else if (item.paramAType == MCUMainFlow.VAR_OR_CONST) {
            // Variable oder Konstante
            if (isConst(paramA.getValue())) {
                return "PUSH_SI_C " + paramA.getValue();
            } else if (isVariable(paramA.getValue())) {
                return "LOAD_I " + paramA.getValue();
            }
        }
    ...
    private Function_Item addFunction(String functionName, int paramCount, int paramAType, int paramBType,
            boolean hasResult, boolean directParam) {
    ...
    public void init() {
        ...
        // addFunction(String functionName, int paramCount, int paramAType, int paramBType, boolean
        // hasResult, boolean directParam)
