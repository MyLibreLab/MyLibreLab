...
package elements.circuit.image.canny;// *****************************************************************************
...
public class Canny extends Thread {
    ...
    private int suppress(int[][] m_2d, int sector, int i, int j, int lowthresh) {
        ...
        // if (318 < i && i < 322 && 113 < j && j < 117)System.out.println("ij("+i+","+j+") sector:
        // "+sector+" neigh: "+m_2d[i-1][j-1]+" "+m_2d[i-1][j]+" "+m_2d[i-1][j+1]+" "+m_2d[i][j-1]+"
        // "+m_2d[i][j]+" "+m_2d[i][j+1]+" "+m_2d[i+1][j-1]+" "+m_2d[i+1][j]+" "+m_2d[i+1][j+1]);
