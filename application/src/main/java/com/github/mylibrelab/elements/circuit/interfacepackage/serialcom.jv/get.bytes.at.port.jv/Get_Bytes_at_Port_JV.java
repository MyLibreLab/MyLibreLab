...
package com.github.mylibrelab.elements.circuit.interfacepackage.serialcom.jv.get.bytes.at.port.jv;
// ********************************
...
public class Get_Bytes_at_Port_JV extends JVSMain {
    ...
    public void process() {
        ...
        if ((EnableProccess && Error_in.getValue() == false)) // Only executes one time if no error and Rising Edge
                                                              // Enable Input.
        {
            EnableProccess = false;
            System_out.setValue("");
            ...
            try {
                ...
                if (vsPortIn != null) {
                    ...
                    Thread.sleep(DelayBeforeRead.getValue());
                    ...
                    if (Debug_Window_En_in.getValue()) {
                        element.jConsolePrintln(Element_Tag + "_Serial Port: " + SerialPortNameIn.getValue()
                                + "_TOTAL_BYTES:" + BytesAtPortOut.getValue());
                    }
