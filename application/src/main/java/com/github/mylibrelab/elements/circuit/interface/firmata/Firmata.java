...
package com.github.mylibrelab.elements.circuit.Interface.Firmata.src;// *****************************************************************************
        ...
        for (String line : allowedLines) {
            ...
            if (line.length() > 0) {
                ...
                if (cols.length == 2) {
                    ...
                    if (pinType.trim().startsWith("DIGITAL_OUTPUT") || pinType.trim().startsWith("PWM_OUTPUT")
                            || pinType.trim().startsWith("SERVO_OUTPUT")) {
                        inputPins++;
                    }
                    if (pinType.trim().startsWith("DIGITAL_INPUT") || pinType.trim().startsWith("INPUT_PULLUP")
                            || pinType.trim().startsWith("ANALOG_INPUT")) {
                        outputPins++;
                    }
                }
            }
            ...
            for (String line : allowedLines) {
                ...
                if (cols.length == 2) {
                    ...
                    if (pinType.trim().startsWith("SERVO_OUTPUT")) {
                        ...
                        // System.out.println("XXX : "+pinType.substring("SERVO_OUTPUT".length()+1,
                        // pinType.length()-1));
                        String splitted[] =
                                pinType.substring("SERVO_OUTPUT".length() + 1, pinType.length() - 1).split(",");

                        if (splitted.length == 2) {
                            setServoConfig(pinNum, Integer.parseInt(splitted[0].trim()),
            ...
            if (cc == 0xF7) {
                ...
                if (params[0] == 0x6A) {
                    ...
                    if (counter > 5) {
                        ...
                        System.out.println(
                                "Firmata Protocol Version: " + version_1 + "." + version_2 + " (" + version_str + ")");
                    }
