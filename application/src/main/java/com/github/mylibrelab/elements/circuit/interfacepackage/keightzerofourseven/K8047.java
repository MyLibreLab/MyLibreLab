...
package com.github.mylibrelab.elements.circuit.interfacepackage.keightzerofourseven;// *****************************************************************************
...
public class K8047 extends JVSMain implements MyOpenLabDriverOwnerIF {
    ...
    private final VSDouble outCH1 = new VSDouble(0);
    private final VSDouble outCH2 = new VSDouble(0);
    private final VSDouble outCH3 = new VSDouble(0);
    private final VSDouble outCH4 = new VSDouble(0);
    ...
    public void getCommand(String commando, Object value) {
        ...
        if (value instanceof Double) {
            ...
            if (commando.equals("DAC1")) {
                outCH1.setValue(val.doubleValue());
                element.notifyPin(0);
            } else if (commando.equals("DAC2")) {
                outCH2.setValue(val.doubleValue());
                element.notifyPin(1);
            } else if (commando.equals("DAC3")) {
                outCH3.setValue(val.doubleValue());
                element.notifyPin(2);
            } else if (commando.equals("DAC4")) {
                outCH4.setValue(val.doubleValue());
                element.notifyPin(3);
            }
        }
    }
    ...
    public void start() {
        ...
        if (dllsInstalled()) {
            ...
            showMessage("Please copy K8047e.exe, K8047D.dll and FASTTime32.dll from your Driver CD/DVD in " + winDir
                    + " Directory!");
        }
    ...
    public void elementActionPerformed(ElementActionEvent evt) {
        ...
        switch (idx) {
            case 4:
                driver.sendCommand("SET_GAIN_CH1", new Integer(gainCH1.getValue()));
                break;
            case 5:
                driver.sendCommand("SET_GAIN_CH2", new Integer(gainCH2.getValue()));
                break;
            case 6:
                driver.sendCommand("SET_GAIN_CH3", new Integer(gainCH3.getValue()));
                break;
            case 7:
                driver.sendCommand("SET_GAIN_CH4", new Integer(gainCH4.getValue()));
                break;
            case 8: {
