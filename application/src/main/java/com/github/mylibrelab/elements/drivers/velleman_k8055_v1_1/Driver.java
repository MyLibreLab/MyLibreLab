...
package com.github.mylibrelab.elements.drivers.velleman_k8055_v1_1;/*
                                                                                  * Copyright (C) 2020 MyLibreLab
...
public class Driver implements MyOpenLabDriverIF {
    ...
    public void sendCommand(String commando, Object value) {
        ...
        if (value == null) {
            if (commando.equals("RESET_COUNTER_1"))
                lib.ResetCounter(new NativeLong(1));
            else if (commando.equals("RESET_COUNTER_2")) lib.ResetCounter(new NativeLong(2));
        } else if (value instanceof Boolean) {
            Boolean val = (Boolean) value;

            if (commando.equals("out1"))
                setDigitalChannelValue(1, val);
            else if (commando.equals("out2"))
                setDigitalChannelValue(2, val);
            else if (commando.equals("out3"))
                setDigitalChannelValue(3, val);
            else if (commando.equals("out4"))
                setDigitalChannelValue(4, val);
            else if (commando.equals("out5"))
                setDigitalChannelValue(5, val);
            else if (commando.equals("out6"))
                setDigitalChannelValue(6, val);
            else if (commando.equals("out7"))
                setDigitalChannelValue(7, val);
            else if (commando.equals("out8")) setDigitalChannelValue(8, val);

        } else if (value instanceof Integer) {
            Integer val = (Integer) value;

            if (commando.equals("ADC1"))
                setAnalogChannelValue(1, val);
            else if (commando.equals("ADC2")) setAnalogChannelValue(2, val);
        }
    }
            ...
            public void actionPerformed(ActionEvent evt) {
                if (isOpen) {
                    try {
                        ...
                        if (counter1.intValue() != oldCounter1) {
                            oldCounter1 = counter1.intValue();
                            owner.getCommand("COUNTER1", new Integer(counter1.intValue()));
                        }
                        if (counter2.intValue() != oldCounter2) {
                            oldCounter2 = counter2.intValue();
                            owner.getCommand("COUNTER2", new Integer(counter2.intValue()));
                        }

                        if (A1 != oldA1) {
                            oldA1 = A1;
                            owner.getCommand("DAC1", new Integer((int) A1));
                        }
                        if (A2 != oldA2) {
                            oldA2 = A2;
                            owner.getCommand("DAC2", new Integer((int) A2));
                        }
