...
package com.github.mylibrelab.elements.front.twoautomation.industrial.buttons.jv.square.led.ind.jv;
...
public class LEDPanel extends JVSMain implements PanelIF {
    ...
    public void paint(java.awt.Graphics g) {
        if (element != null) {
            ...
            g2.fillRoundRect(X0, Y0, tWidth, tHeight, 40, 40);

            g2.setColor(edgeColor.getValue());
            g2.setStroke(strokeLine);
            g2.drawRoundRect(X0, Y0, tWidth, tHeight, 40, 40);
            ...
            g2.drawRoundRect(X0 + (TempStroke / 2), Y0 + (TempStroke / 2), tWidth - TempStroke, tHeight - TempStroke,
                    20, 20);
            ...
            g2.setColor(edgeColor.getValue());
            g2.drawRoundRect(X0, Y0, tWidth, tHeight, 40, 40);
            // g2.drawRoundRect(X0+(TempStroke/8), Y0+(TempStroke/8), tWidth-(TempStroke/4),
            // tHeight-(TempStroke/4),20,20);

            SVGObject light = svgManager.getSVGObject("light");
            SVGObject blackim = svgManager.getSVGObject("rect1871");
            SVGObject bubble = svgManager.getSVGObject("path2762");
            bubble.setVisible(true);
            blackim.setVisible(false);
            light.setVisible(true);

            if (on)
                light.setFillColor(onButtonColor.getValue());
            else
                light.setFillColor(offButtonColor.getValue());

            int X0Im = (int) (r.width * 0.62);
            int Y0Im = (int) (r.height * 0.95);

            if (light != null) {

                light.translate(42, -9);
                bubble.translate(42, -9);
                // blackim.translate(23, 23);

                svgManager.paint(g2, (int) X0Im, (int) Y0Im);
            }
    ...
    public void init() {
        ...
        element.jSetMinimumSize(100, 100);
