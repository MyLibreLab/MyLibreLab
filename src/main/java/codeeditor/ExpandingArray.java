...
/**
 * Simple expanding array. The array is expandable by amortized doubling with number of valid items
 * stored in count field with minimum MIN_SIZE.
 */
public class ExpandingArray {
    ...
    /**
     * Shifts a part of the array starting from shiftStart by shiftLength elements to the right. Expands
     * the array if necessary. Inserts null elements to empty positions.
     *
    ...
    public void shift(int shiftStart, int shiftLength) {
        ...
        if (new_items != items) System.arraycopy(items, 0, new_items, 0, shiftStart);

        // elements following inserted ones:
        System.arraycopy(items, shiftStart, new_items, shiftStart + shiftLength, count - shiftStart);

        if (new_items == items && count > shiftStart) fill(shiftStart, shiftStart + shiftLength - 1, null);
    ...
    /**
     * Removes shiftLength elements starting from shiftStart one shifting following elements to the
     * left. If the array is to long (its valid elements occupies its fourth or less) than it is
     * shortened to one half.
     *
     * @param shiftStart the first element index to be shifted
     * @param shiftLength the number of elements to insert before the first shifted one
     */
