    ...
    timer = new javax.swing.Timer(pollTime.getValue(), new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt)
          {
            if (started && driver!=null)
            {
              ...
              if (led5.getValue()) value|=(1<<4); else value&=~(1<<4);
              if (led6.getValue()) value|=(1<<5); else value&=~(1<<5);
    ...
    if (value instanceof VS1DByte)
    {
      ...
      if (commando.equals("BYTESRECEIVED"))
      {
        ...
        if (len==13)
        {
          ...
          if (t5!=taste5.getValue()) {taste5.setValue(t5);element.notifyPin(4);}
          if (t6!=taste6.getValue()) {taste6.setValue(t6);element.notifyPin(5);}
