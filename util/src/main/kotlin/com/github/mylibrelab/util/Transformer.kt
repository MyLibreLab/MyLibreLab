...
private const val defaultDelimiter: Char = ','
private const val defaultDelimiterAlt: Char = '='
...
fun <T, ListType> listParser(
    ...
    write = { value ->
        when (value.isNotEmpty()) {
            true -> collectionConstructor(value.split(delimiter).map { elementParser.write(it) })
            else -> collectionConstructor(listOf())
        }
    },
    read = { list ->
...
fun splitString(delimiter: Char = defaultDelimiter): Transformer<String, List<String>> =
    listParser(identityTransformer(), delimiter).reverse()
